...
for i in range(sample_num):  
	if(i%2==0):
		rotate_face = ndimage.rotate(train_in[i], 10 , reshape =False ) 
	else:
		rotate_face = ndimage.rotate(train_in[i], -10, reshape =False  ) 
	rotate_face = rotate_face.reshape(1,48,48,1)
	train_in=np.concatenate( (train_in,rotate_face )  , axis = 0)  
 
np.save('test_angle_1', train_in ) 
total = train_in.shape[0] 
  
vi = train_in[:int(total * 0.07)]
vo = train_in[:int(total * 0.07)]
train_in = train_in[int(total * 0.07):]
train_out = train_out[int(total * 0.07):]
...
model.fit(train_in, train_out, epochs=38, batch_size=200,validation_data=(vi,vo))
